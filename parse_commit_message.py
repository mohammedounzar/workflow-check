...
def parse_commit_message(commit_msg):
    ticket_nbr = re.findall(r'\b[E]{1,2}\s*[E]?\s*[V]{0,2}\s*2\s*-?\s*\d+\b', commit_msg, re.IGNORECASE)  #  EEV2-1234  , \bEEV2-\d+\b
    ...
    sheet_name = re.findall(r'\b[r]?[R]?elease\s*-?\s*\d+\.\d+\.\d+\b', commit_msg)  # release-1.2.3   
